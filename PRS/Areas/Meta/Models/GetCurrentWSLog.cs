...
namespace IP.Areas.Meta.Models
{
    public class GetCurrentWSLog
    {
        ...
        public List<Hashtable> lstCurrentWSLog { get; set; }
        ...
        public bool GetList(string frmDt, string toDate, string SerialNo, string testerName)
        {
  ...
  ";
            if (conType == "PROD")
            {
                query += @"FROM PlusRS.tia.GetUnitCurrentWSLog ";
            }
            else 
            {
                query += @"FROM PlusRS.tia.GetUnitCurrentWSLog_Test ";
            }
            ...
            cLog oLog = new cLog();
            oLog.AddSqlQuery("182", query, string.Empty, false);
            ...
            {
                if (dt.Rows.Count > 0)
                    lstCurrentWSLog = cCommon.ConvertDtToHashTable(dt);
                return true;
