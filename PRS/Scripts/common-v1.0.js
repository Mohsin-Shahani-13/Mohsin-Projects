...
function ShowOption(menuTitle, menuUrl, designedBy, fromIndex, mailTo) {
...
function MakeMeGrid(tableId, data, tableHeight, columnWidth, search, isScrollX, footerFunction, isDetailed, reportTitleId, rowSelection) {
    ...
    if (isDetailed == true) {
        ...
        $('#buttons').empty();
        var qrytool = '<a id="showQuery" class="btn-header btn-box-tool" href="\\SqlDocs\\Index?rptCode=' + rptDtlCode + '&rptTitle=' + rptDtlTitle + '" title="How they do that" target="_blank">';
        ...
        if (rptCode == undefined) {
            ...
            $('#buttons').empty();
            var qrytool = '<a id="showQuery"  class="btn-header btn-box-tool" href="\\SqlDocs\\Index?rptCode=' + rptCode + '&rptTitle=' + rptTitle + '" title="How they do that" target="_blank">';
            qrytool += '<span class="fa-stack sqlDoc"><i class="fa fa-circle fa-stack-2x" style="color: rgb(221, 75, 57);"></i><i class="fa fa-database fa-stack-1x" style="color: white;"></i></span>';
            qrytool += '</a>';
            $('#buttons').append(qrytool);
        }
...
function MakeMeBabyGrid(tableId, data, tableHeight, columnWidth, buttonid, isScrollX, reportTitle, footerFunction) {
    isScrollX = (isScrollX == undefined ? false : true);
    var isFooter = (footerFunction == undefined ? false : true);
    var drFlag;
    var order = [];
...
function AddSqlDoc(buttonId, gridButtonSize, isBaby) {
    ...
    if (rptCode == undefined) {
        ...
        var rptDtlTitle = reportTitle.substring(0, reportTitle.indexOf('>'));
        $('#' + buttonId).empty();
        var qrytool = '<a  class="' + className + '" id="showQuery"  href="\\SqlDocs\\Index?rptCode=' + rptDtlCode + '&rptTitle=' + rptDtlTitle + '&isDetail=Y' +  '" title="How they do that" target="_blank">';
        ...
        $('#' + buttonId).empty();
        var qrytool = '<a class="' + className + '" id="showQuery" href="\\SqlDocs\\Index?rptCode=' + rptCode + '&rptTitle=' + rptTitle + '&isDetail=N' +  '" title="How they do that" target="_blank">';
        qrytool += '<span><span><span class="fa-stack sqlDocFrameGrid" ><i class="fa fa-circle fa-stack-2x" style="color: rgb(221, 75, 57);"></i><i class="fa fa-database fa-stack-1x" style="color: white;"></i></span></span></span>';
        qrytool += '</a>';
        $('#' + buttonId).append(qrytool);
    }
...
function OpenPage(_url, designedBy, mailTo) {
    if (designedBy != undefined) {
        $('.modal-footer > #designedby').empty();
        $('.desgndBy').html('<div id="lstMenus_info" role="status" aria-live="polite">' + designedBy + ' | <a id="MailTo" href="' + mailTo + '"  target="_blank" title="Click to report features and bugs">MailTo</a> |  <a id="ActivityLog" href="#" onclick="ShowLog()" title="Click to View Activity Log">Activity Log</a> </div>');
    }
    $.ajax({
        url: _url,
...
function Navigate(report_target, report_URL, report_code, report_title, report_designed_by, mailTo) {
    
    if (report_URL.indexOf("contractId") > 1)
        report_URL += '&rptCode=' + report_code;
    else
        report_URL += '?rptCode=' + report_code;
    
    if (report_target == "option") {
        ShowOption(report_title, report_URL, report_designed_by, undefined, mailTo);
    }    
    else {
        report_URL += "&menuTitle=" + report_title;        
        OpenPage(report_URL, report_designed_by, mailTo);
    }
...
function Remove_thousand_separator(num) {
...
function ShowLog() {    
    var url = "@Url.Action('ActivityLog', 'LogQueries')";
    var rptCode = $("#rptcode").val();
    var rptTitle = $("#rptTitle").val();
    window.open(url + "?rptCode=" + rptCode + "&rptTitle=" + rptTitle, '_blank');
}
