...
namespace IP.Models
{
    public class DBIngenicoThird
    {
       ...
       to_do, 
       wip 
FROM   [DW].[rc_dw].[dw_ops_morning_ignecio_fact] ";
...
FROM   [DW].[rc_dw].[dw_ops_morning_ignecio_fact] ";
       ...
       to_do, 
       wip 
FROM   [DW].[rc_dw].[dw_ops_afternoon_ignecio_fact] ";
...
FROM   [DW].[rc_dw].[dw_ops_morning_ignecio_fact] ";
       ...
       to_do, 
       wip 
FROM   [DW].[rc_dw].[dw_ops_night_ignecio_fact] ";
                dtIngenico = new DataTable();
                dtIngenico = oDAL.GetData(query);
                string query2 = @"SELECT DISTINCT Format(refresh_time, 'yyyy.MM.dd HH:mm') AS REFRESH_TIME,
                [shift] 
FROM   [DW].[rc_dw].[dw_ops_night_ignecio_fact] ";
            ...
            if (!oDAL.HasErrors)
            {
                lstIngenico = cCommon.ConvertBATDtToArrayList(dtIngenico);
